...
//	else if (area >= 200 && !rand_int(2)) has_moat = 1;
	...
	floor_info = CAVE_ICKY | CAVE_ROOM | CAVE_GLOW;
	floor_info2 = 0x0;
...
static void wild_add_hotspot(struct worldpos *wpos) {
	...
	*/
	...
	case WILD_GRASSLAND:
		/* sometimes a pond */
		if (rand_int(100) < 50) hot_terrain.water = 1000;
		/* otherwise a glade */
		else {
	...
	case WILD_FOREST:
		/* sometimes a pond */
		if (rand_int(100) < 60) hot_terrain.water = 1000;
	...
	case WILD_DENSEFOREST:
		...
		else if (rand_int(100) < 70) hot_terrain.water = 1000;
	...
	case WILD_SWAMP:
		/* sometimes a pond */
		if (rand_int(100) < 40) hot_terrain.water = 1000;
		/* otherwise a mud pit */
		else {
	...
	case WILD_ICE:
		/* sometimes a pond */
		if (rand_int(100) < 30) hot_terrain.water = 1000;
...
/* Is the [x,y] pos on a specific worldmap sector currently within actual weather elements being drawn? */
bool pos_in_weather(struct worldpos *wpos, int x, int y) {
	int j, dx, dy, d;
