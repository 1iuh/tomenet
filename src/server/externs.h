...
extern char *k_name;
extern char *k_text;
extern u16b k_info_num[MAX_K_IDX], tr_info_rev[MAX_TR_IDX];
...
extern bool (*ang_sort_extra2_comp)(int Ind, vptr i, vptr j, vptr k, vptr m, int a, int b);
...
extern char priv_note[MAX_NOTES][MSG_LEN], priv_note_sender[MAX_NOTES][NAME_LEN], priv_note_target[MAX_NOTES][NAME_LEN], priv_note_u[MAX_NOTES][MSG_LEN], priv_note_date[MAX_NOTES][NAME_LEN];
...
extern int projectable_get_m_idx(int Ind, int y1, int x1, int y2, int x2, int range);
...
extern s16b brand_dam_aux(int Ind, object_type *o_ptr, int slot, int tdam, monster_type *m_ptr, bool thrown);
extern s16b brand_dam_aux_player(int Ind, object_type *o_ptr, int slot, int tdam, player_type *p_ptr, bool thrown);
...
extern void do_nazgul(int Ind, int *k, monster_race *r_ptr, int slot);
extern void set_black_breath(int Ind, int cause);
...
extern s16b auto_stow(int Ind, object_type *o_ptr, int o_idx, bool pick_one, bool store_bought, bool quiet, u32b cave_info);
#endif
...
extern s16b inven_takeoff(int Ind, int item, int amt, bool called_from_wield, bool force);
...
extern bool subinven_can_accept(int Ind, object_type *i_ptr, int sslot, bool store_bought);
extern bool item_matches_subinven(int Ind, int subinven_group_type, object_type *o_ptr);
...
extern bool subinven_group_player(int Ind, int group, int slot);
 #endif
extern bool do_cmd_subinven_fill(int Ind, int slot, bool quiet);
#endif
extern int bodymonster_hands(int Ind);

/* cmd4.c */
extern void do_cmd_check_artifacts(int Ind, int line, char *srcstr);
extern void do_cmd_check_uniques(int Ind, int line, char *srcstr, int mode, int Ind2);
...
extern void do_cmd_show_houses(int Ind, bool local, bool own, s32b id);
...
extern int can_use_wordofrecall(player_type *p_ptr);
...
extern int Send_char_info(int Ind, int race, int class, int trait, int sex, u32b mode, int lives, cptr name);
...
extern int Send_bpr_wraith_prob(int Ind, byte bpr, byte attr, cptr bpr_str);
...
extern int Send_store(int ind, s16b pos, byte attr, int wgt, int number, int price, cptr name, byte tval, byte sval, s32b pval, char *powers);
extern int Send_store_wide(int ind, s16b pos, byte attr, int wgt, int number, int price, cptr name, byte tval, byte sval, s32b pval,
...
extern int Send_store_special_clr_force(int Ind, char line_start, char line_end);
extern int Send_store_special_anim(int Ind, u16b anim1, u16b anim2, u16b anim3, u16b anim4);
extern int Send_store_info(int ind, int num, cptr store, cptr owner, int items, int purse, byte attr, char c);
extern int Send_store_action(int ind, char pos, u16b bact, u16b action, cptr name, byte attr, char letter, s32b cost, u32b flag);
...
extern int privilege(char *name, int level);
...
extern void party_msg_ignoring2(int sender, int party_id, cptr msg, cptr msg_u);
...
extern bool add_hostility(int Ind, cptr name, bool initiator, bool admin_forced);
extern bool remove_hostility(int Ind, cptr name, bool admin_forced);
...
extern void add_player_name(cptr name, int id, u32b account, byte race, byte class, u32b mode, byte level, byte max_plv, u16b party, byte guild, u32b guild_flags, u16b xorder, time_t laston, byte admin, struct worldpos wpos, char houses, byte winner, byte order);
extern void verify_player(cptr name, int id, u32b account, byte race, byte class, u32b mode, byte level, u16b party, byte guild, u32b guild_flags, u16b quest, time_t laston, byte admin, struct worldpos wpos, char houses, byte winner, byte order);
...
extern u32b lookup_player_mode(int id);
...
extern bool detect_treasure_object(int Ind, int rad);
...
extern bool fire_wall_wide(int Ind, int typ, int dir, int dam, int time, int interval, char *attacker);
...
extern void sound_near_area(int y, int x, int rad, worldpos *wpos, cptr name, cptr alternative, int type);
...
extern int gold_colour(s32b amt, bool fuzzy, bool compact);
...
extern cptr compat_mode(u32b mode1, u32b mode2);
...
extern void empty_subinven(int Ind, int item, bool drop, bool quiet);
...
extern void ang_sort_extra2_aux(int Ind, vptr i, vptr j, vptr k, vptr m, int p, int q);
extern void ang_sort_extra2(int Ind, vptr i, vptr j, vptr k, vptr m, int n);
extern void ang_sort_extra2_swap_distance(int Ind, vptr i, vptr j, vptr k, vptr m, int a, int b);
extern bool ang_sort_extra2_comp_distance(int Ind, vptr i, vptr j, vptr k, vptr m, int a, int b);
...
extern bool get_outward_target(int Ind, int *x, int *y, int maxdist, int avoid_dir, bool any_grid, bool skip_sleeping, bool skip_friendly);
extern int determine_dir(int sx, int sy, int dx, int dy);
...
extern void kill_house_contents(int h_idx);
...
extern void casino_result(int Ind, bool win, bool tie, bool deduct_loss);
extern int draw_card(int Ind, int *colour, int *value);
...
extern void quest_check_goal_k(int Ind, int m_idx);
...
extern s32b fake_waitpid_geo, fake_waitpid_ping, fake_waitpid_route, fake_waitpid_clver, fake_waitpid_clver_timer;
...
extern int mtdoom_wpos_x, mtdoom_wpos_y, mtdoom_wpos_z;
extern unsigned char nether_realm_collapsing;
...
extern int dbgvar1, dbgvar2, dbgvar3, dbgvar4, dbgvar5, dbgvar6;
