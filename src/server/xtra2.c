...
bool monster_death(int Ind, int m_idx) {
	...
	/* Easteregg, sort of: Killing the Horned Reaper in Dungeon Keeper event gives you a reward worth the 180k you likely spend to achieve this :-p */
		...
		qq_ptr->discount = 99; //workaround -> practically unsalable
#else
		qq_ptr->discount = 100;
#endif
 ...
 #endif
	{
		...
		if (!found_chemical && ((r_ptr->flags3 & RF3_ANIMAL) || r_idx == RI_NOVICE_MAGE || r_idx == RI_NOVICE_MAGE_F || m_ptr->ego == RE_RUNEMASTER)
		    ...
		    /* Avoid item flood */
		    && (!(r_ptr->flags1 & RF1_FRIENDS) || !rand_int(2))) {
...
void player_death(int Ind) {
	...
	/* For the log file and therefore also for the "recent deaths" list:
	...
	if (xorders[i].flags & QUEST_GUILD) {
		...
		if (val < 1000 && rand_int(3)) { /* eg instead of basic (non-ego) enchanted armour/weapon */
			...
			case 0:
				...
				case 4: invcopy(o_ptr, lookup_kind(TV_SCROLL, SV_SCROLL_IDENTIFY)); o_ptr->number = 10 + rand_int(3); break;

				case 5: invcopy(o_ptr, lookup_kind(TV_SCROLL, SV_SCROLL_ENCHANT_ARMOR)); o_ptr->number = 5 + rand_int(2); break;
			...
			case 1:
				...
				case 0:
					switch (rand_int(2)) {
					case 0: invcopy(o_ptr, lookup_kind(TV_POTION, SV_POTION_SLOW_POISON)); o_ptr->number = 7 + rand_int(2); break;
					case 1: invcopy(o_ptr, lookup_kind(TV_POTION, SV_POTION_CURE_POISON)); o_ptr->number = 5 + rand_int(2); break;
					}
					break;
				case 1:
					switch (rand_int(2)) {
					case 0: invcopy(o_ptr, lookup_kind(TV_POTION, SV_POTION_HEROISM)); o_ptr->number = 6 + rand_int(2); break;
			...
			case 0:
				switch (rand_int(8)) {
				case 1: invcopy(o_ptr, lookup_kind(TV_SCROLL, SV_SCROLL_ENCHANT_ARMOR)); o_ptr->number = 16 + rand_int(3); break;
...
//		msg_format(Ind, "Gain=%d, skill=%d", gain, skill_trauma);
	...
	if (r_ptr->flags7 & RF7_NO_DEATH) {
		...
		if (p_ptr->instakills != 2) {
			p_ptr->test_dam += dam; //hack: Target dummy damage counts fully, or we couldn't test!
			return(FALSE);
		}
...
void monster_death_mon(int am_idx, int m_idx) {
	...
	for (j = 0; j < number; j++) {
		...
		for (i = 0; i < 20; ++i) {
			...
			/* Place Gold */
			if (do_gold && (!do_item || (rand_int(100) < 50)))
				place_gold(m_ptr->owner, wpos, ny, nx, 10, 0);
			/* Place Object */
			else {
				place_object_restrictor = RESF_NONE;
...
bool get_outward_target(int Ind, int *x, int *y, int maxdist, int avoid_dir, bool any_grid, bool skip_sleeping, bool skip_friendly) {
	...
	if (!any_grid) while (d <= maxdist) {
		...
		for (offset_grid = rndoffset; offset_grid < ringlength + rndoffset; offset_grid++) {
			...
			if ((grid + d / 2) / d == avoid_dir || (grid + (d - 1) / 2) / d == avoid_dir) continue;
