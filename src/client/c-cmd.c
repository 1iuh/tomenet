...
//#define CTRLC_DEBUG /* Use CTRL-C for some special debug stuff instead of its normal function (audio shortcut)? */
	...
	case KTRL('C'):
		...
		tiles_rawpict_org[0].y = 768;
		tiles_rawpict_org[0].w = 128;
		tiles_rawpict_org[0].h = 192;
...
#ifdef ENABLE_SUBINVEN
	if (!c_get_item(&item, "Eat what? ", (USE_INVEN | USE_EXTRA | NO_FAIL_MSG | USE_SUBINVEN))) {
#else
...
void cmd_load_pref(void) {
	...
	if (!process_pref_file_aux(buf, buf, FALSE)) /* Success? */
		c_msg_format("Loaded file %s", buf);
	return;
}
